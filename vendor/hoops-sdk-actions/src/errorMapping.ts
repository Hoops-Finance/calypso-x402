...
const ACCOUNT_ERROR_MAP: Record<number, HoopsErrorCode> = {
  1: HoopsErrorCode.ALREADY_INITIALIZED,
  2: HoopsErrorCode.AUTH_FAILED,
};

const ROUTER_ERROR_MAP: Record<number, HoopsErrorCode> = {
...
const ERROR_PATTERNS: [RegExp, HoopsErrorCode][] = [
...
export function normalizeHoopsError(e: unknown): HoopsError {
  if (e instanceof HoopsError) return e;
  ...
  if (code !== null) {
    const mapped =
      ACCOUNT_ERROR_MAP[code] ?? ROUTER_ERROR_MAP[code] ?? HoopsErrorCode.UNKNOWN;
