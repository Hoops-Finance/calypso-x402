...
import type { HoopsNetwork, LpPlan, DepositParams } from "hoops-sdk-types";
import {
...
export async function addLiquidity(
  ...
  const { xlm: xlmBal, usdc: usdcBal } = await getStandardBalances(
    server,
    config.passphrase,
    pubkey,
    accountId,
  ...
  if (usdcBal < TX_DEFAULTS.minUsdcForDeposit) {
    throw new Error(
      `Insufficient USDC: ${formatBalance(usdcBal)} (need >= ${formatBalance(TX_DEFAULTS.minUsdcForDeposit)})`
    );
  }
  ...
  if (availableXlm <= 0n) {
    throw new Error(
      `Insufficient XLM after reserving fees: ${formatBalance(xlmBal)}`
    );
  }
  ...
  if (adapterIds.length === 1) {
    ...
    const tx = await account.buildDepositTx(
      pubkey, addressBook.tokens.usdc, usdcBal, lpPlans, deadline
    );
    ...
    for (const split of splits) {
      ...
      const tx = await account.buildDepositTx(
        pubkey, addressBook.tokens.usdc, split.usdc, lpPlans, deadline
      );
      await signAndSubmitTx(server, keypair, tx);
    }
  }
...
export async function addLiquidity50_50(
  keypair: Keypair,
  accountId: string,
  network: HoopsNetwork
): Promise<void> {
