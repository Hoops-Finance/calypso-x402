...
export async function deploySmartAccount(
  ...
  const deployer = new AccountDeployerContract(
    addressBook.accountDeployer,
    server,
    config.passphrase
  );

  const deployTx = await deployer.buildDeployAccountTx(
    pubkey,
    addressBook.router,
    addressBook.wasmHash
  );
