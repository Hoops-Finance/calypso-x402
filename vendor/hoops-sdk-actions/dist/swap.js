...
import { createRpcClientForNetwork, getNetworkConfig, SmartAccountContract, signAndSubmitTx, toStroops, getDeadline, } from "hoops-sdk-core";
/**
 * Execute a swap through any pool.
 * Use `getBestQuote` or `getAllQuotes` from hoops-sdk-core to find the optimal pool.
 */
export async function swap(keypair, accountId, network, params) {
    ...
    const swapTx = await account.buildSwapTx(pubkey, params.tokenIn, params.tokenOut, params.amount, params.poolAddress, params.deadline ?? getDeadline());
 ...
 */
export async function swapXlmToUsdc(keypair, accountId, xlmAmount, network, poolAddress) {
