...
export function scValToAddress(val) {
    return Address.fromScVal(val).toString();
}
export function buildLpPlanScVal(plan) {
...
export function buildLpPlansVecScVal(plans) {
    return xdr.ScVal.scvVec(plans.map(buildLpPlanScVal));
}
export function decodeSwapQuote(val) {
    const map = val.map();
    if (!map)
        throw new Error("Expected ScVal map for SwapQuote");
    const fields = {};
...
export function decodeOptionSwapQuote(val) {
...
export function calculateConstantProductQuote(amountIn, reserveIn, reserveOut) {
